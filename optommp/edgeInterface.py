...
## PID configuration
    def GetStatusPID(self, loop_number, parameter):
        offset = (O22SIOUT.BASE_PIDSTATUS_RW
                + (O22SIOUT.BASE_PIDSTATUS_HEXBOUND * loop_number)
                + (O22SIOUT.BASE_PIDSTATUS_HEXLENGTH * parameter))
        data = self.ReadBlock(offset, 4)
        return float(self.UnpackReadResponse(data, 'f'))

    def SetConfPID(self, loop_number, parameter, value):
        offset = (O22SIOUT.BASE_PIDVALUES_RW
                + (O22SIOUT.BASE_PIDVALUES_HEXBOUND * loop_number)
                + (O22SIOUT.BASE_PIDVALUES_HEXLENGTH * parameter))
        data = self.WriteBlock(offset, self.PackFloat(value))
        return self.UnpackWriteResponse(data)

    def GetConfPID(self, loop_number, parameter):
        offset = (O22SIOUT.BASE_PIDVALUES_RW
                + (O22SIOUT.BASE_PIDVALUES_HEXBOUND * loop_number)
                + (O22SIOUT.BASE_PIDVALUES_HEXLENGTH * parameter))
        data = self.ReadBlock(offset, 4)
...
if __name__ == "__main__":
    edge = O22MMP()
    result = edge.GetStatusPID(0,0)
    #data = edge.ReadBlock(0xF2000004, 4)
    #result = edge.UnpackReadResponse(data, 'f')
    #result = edge.GetDigitalPointState(0,0)
    #result = edge.GetAnalogPointValue(7,3)
    #result = edge.SetConfPID(0,4,12.0)
    #result = edge.GetConfPID(0, 0)
    print (result)
    print (edge.LastError())
    edge.close()
